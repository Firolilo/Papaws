...
const REPORTS: ReportCfg[] = [
  ...
  { id:"c22", chebotko:"C22", label:"Veterinarios con más consultas",group:"Veterinarios",desc:"Ranking de veterinarios por número de consultas atendidas.", inputType:"none", fetch:()=>reportesApi.c22_veterinariosPorConsultas(), rows:paginaItems, cols:[{key:"nombreCompleto",label:"Veterinario"},{key:"especialidadPrincipal",label:"Especialidad",render:(v)=><Badge tone="sun">{String(v)}</Badge>},{key:"totalConsultas",label:"Consultas",render:(v)=><span className="font-bold text-moss-700">{String(v)}</span>}] },
  // SERVICIOS
  { id:"c11", chebotko:"C11", label:"Servicio por ID",            group:"Servicios",   desc:"Información de un servicio.", inputType:"servicio", inputLabel:"Servicio", fetch:(id)=>reportesApi.c11_servicioPorId(id), rows:(d)=>(d?[d]:[]), cols:[{key:"idServicio",label:"ID",render:(v)=><span className="font-mono text-xs text-ink-400">{shortId(v)}</span>},{key:"nombre",label:"Nombre"},{key:"descripcion",label:"Descripción"},{key:"duracionEstimadaMinutos",label:"Duración (min)"},{key:"precioBase",label:"Precio",render:(v)=>`Bs ${Number(v).toLocaleString("es", { useGrouping: false })}`}] },
  // PRODUCTOS
...
const COLS_TOTALIZABLES: Record<string, { kpi: string; fmt: (n: number) => string }> = {
  precioBase:                { kpi: "Costo total",     fmt: (n) => `Bs ${n.toLocaleString("es", { useGrouping: false })}` },
  costo:                     { kpi: "Costo total",     fmt: (n) => `Bs ${n.toLocaleString("es", { useGrouping: false })}` },
  stockDisponible:           { kpi: "Stock total",     fmt: (n) => n.toLocaleString("es", { useGrouping: false }) },
  cantidadUsada:             { kpi: "Cantidad total",  fmt: (n) => n.toLocaleString("es", { useGrouping: false }) },
  duracionEstimadaMinutos:   { kpi: "Duración total",  fmt: (n) => `${n.toLocaleString("es", { useGrouping: false })} min` },
};
